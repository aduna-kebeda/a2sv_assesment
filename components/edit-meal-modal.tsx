...
import { updateMeal } from "@/lib/store/slices/mealsSlice"
import { UpdateMealSchema, type UpdateMealInput, type Meal } from "@/lib/types"
...
export default function EditMealModal({ isOpen, onClose, meal }: EditMealModalProps) {
  ...
  const onSubmit = async (data: UpdateMealInput) => {
    try {
      await dispatch(updateMeal(data)).unwrap()
      onClose()
    } catch (error) {
